...
"""
FIXES TO MAKE:
1. fix RESPONSES_PER_BATCH and NUM_GRAD_ACCUMULATION_EXAMPLES
2. update GRPO to make more clear
3. alter save_model code
"""
...
def memory_isolated_train_step(generation_outputs, iteration_list, epoch, result_queue):
    ...
    if epoch != 0:
        model = Qwen3ForCausalLM.from_pretrained(f"./cshs_checkpoints/off_policy_checkpoint_{epoch - 1}",
                                                 quantization_config=quantization_config)
    else:
        model = Qwen3ForCausalLM.from_pretrained("Qwen/Qwen3-8B", quantization_config=quantization_config)
    ...
    for LOGGING_COUNTER_ONLY, iteration_variable in enumerate(iteration_list):
        ...
        for i, chat_pair in enumerate(chat_logs):
            ...
            try:
                ...
                print(
                    f"Error calculating reward for FEN {base_fen}, response '{model_response}': {e}. Assigning penalty.")
                reward_value = -5.0
        ...
        for i in range(len(rewards)):
            ...
            full_text_mask = full_text_tokenized["attention_mask"] #what is this for?
            ...
            with torch.no_grad():
                ...
                base_log_probs = torch.nn.functional.log_softmax(
                    base_model_output.logits[:, input_length - 1:-1, :].to(torch.float32), dim=-1)
            ...
            policy_log_probs = torch.nn.functional.log_softmax(
                policy_model_output.logits[:, input_length - 1:-1, :].to(torch.float32), dim=-1)
            policy_model_probs = torch.exp(policy_log_probs)
            response_slice = generation_slice[:, input_length:]
            selected_policy_probs = policy_model_probs[
                0, torch.arange(policy_model_probs.shape[1]), response_slice[0]].unsqueeze(0)
            selected_base_probs = base_model_probs[
                0, torch.arange(base_model_probs.shape[1]), response_slice[0]].unsqueeze(0)
            ...
            kl_divergence = torch.sum(
                torch.maximum(torch.log(selected_policy_probs + 1e-9) - torch.log(selected_base_probs + 1e-9),
                              torch.zeros_like(selected_base_probs)))
            ...
            result_queue.put({"loss": loss.item(), "kl": kl_divergence.item(),
                              "clipped_ratio": torch.prod(clipped_policy_ratio).item(),
                              "unclipped_ratio": torch.prod(unclipped_policy_ratio).item(),
